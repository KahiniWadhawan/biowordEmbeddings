...
function Senna:train()
	...
	feval=function(x)
		...
		for bi=1,#self.batch do
			local input=self.batch[bi]  --now input is table of {x_pos,x_rnd}
			--local label=self.label_tensors[bi]
			--in our case - label = y = 1
...
function Senna:build_model(config)
	...
	local seql2=seql1:clone('weight','bias')
	...
	self.criterion=nn.MarginRankingCriterion(0.5)
